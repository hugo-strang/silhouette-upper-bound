...
def table_row(dataset: str, metric: str):
    ...
    # Weighted
    if n > 1000:
        weighted_dict = utils.hierarchical_optimized(data=data, metric=metric, method='weighted', t_range=range(2,51))
    else:
        weighted_dict = utils.hierarchical_optimized(data=data, metric=metric, method='weighted', t_range=range(2,n))

    # Single
    if n > 1000:
        single_dict = utils.hierarchical_optimized(data=data, metric=metric, method='single', t_range=range(2,51))
    else:
        single_dict = utils.hierarchical_optimized(data=data, metric=metric, method='single', t_range=range(2,n))
    ...
    return [dataset, metric, weighted_str, single_str, kmeans_str, ub_dict['ub'], ub_dict['min'], ub_dict['max']]
