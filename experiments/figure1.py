...
def graph(params):
    ...
    for k in range(2, 21):
        ...
        # Kmeans
        kmeans_scores = kmeans_optimized(
            data=X, k_range=range(k, k + 1), n_init=10
        )['best_scores']
    ...
    df_melted = df.melt(id_vars="K", 
                        value_vars=["KMeans Silhouette", "Weighted-Linkage Silhouette", "Single-Linkage Silhouette"],
                        var_name="Method", 
                        value_name=" ")
    ...
    plt.axhline(y=ref, color="black", linestyle="--", linewidth=1.5, label=f"Upper bound")
    plt.axhline(y=ref_80, color="red", linestyle="--", linewidth=1.5, label=rf"Upper bound ($\kappa$={80})")
